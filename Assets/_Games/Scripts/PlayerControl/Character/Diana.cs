...
public class Diana : Character
{
    public Axe slashRiven;
    ...
    {
        //TriggerSkill();
        Axe fb = PathologicalGames.PoolManager.Pools["Projectile"].Spawn(slashRiven.transform).GetComponent<Axe>();
        fb.transform.position = shootPos.position;
        ...
        if (PlayerManager.Instance.transform.localScale.x == 1)
        {
            fb.transform.localScale = new Vector3(1.2f, 1.2f, 1.2f);
