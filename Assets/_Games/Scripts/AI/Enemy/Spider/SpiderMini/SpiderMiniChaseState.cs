...
{
    public class SpiderMiniChaseState : State
    {
        private MiniSpider enemy;
        private int currentDirection;
        public SpiderMiniChaseState(MiniSpider controller, string stateName) : base(controller, stateName)
        {
        ...
        public override void UpdatePhysic()
        {
            ...
            else
            {
                ...
                if (Vector2.Distance(target.transform.position, enemy.transform.position) <= enemy.attackRange)
                {
                    enemy.SwitchState(enemy.spiderMiniAttackState);
                }
            }
        }
    }
