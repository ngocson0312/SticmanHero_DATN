...
{
    public class EndermanChaseState : State
    {
        private Enderman enemy;
        private int currentDirection;
        public EndermanChaseState(Enderman controller, string stateName) : base(controller, stateName)
        {
        ...
        public override void UpdatePhysic()
        {
            ...
            else
            {
                ...
                if (Vector2.SqrMagnitude(target.transform.position - enemy.transform.position) <= enemy.attackRange)
                {
                    ...
                    enemy.SwitchState(enemy.endermanAttackState);
                }
            }
        }
    }
